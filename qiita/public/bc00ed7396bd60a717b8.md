...
リンクは、リンクテキストとリンク先で構成されており、リンクタイトルも設定できます。
...
上記のような文を **インラインリンク** を用いて表現する際は下記のように記述します。
...
URI の前後に改行を含めると URI 先の タイトルと OGP 画像を取得し表示します。
