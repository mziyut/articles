...
:qiitan: Qiitan (Ruboty) で GitHub の issue/pull request を検索したい場面がありました。
"ruboty-qiita-github" に `search issues` command を追加しました。
...
今回は、 `usrt:mziyut`, `is:open`, `is:public` の issue/pull request を 1 件取得する条件で issue/pull request を検索しました。
